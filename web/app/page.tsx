...
function Dashboard({ user }: { user: User }) {
  ...
  return <main className="app-shell">
    ...
    <section className="workspace-grid">
      ...
      {showSettings && <div className="lightbox" role="dialog" aria-modal="true" aria-label="Paramètres et tableaux" onClick={() => setShowSettings(false)}><div className="lightbox-panel tables-panel" onClick={(event) => event.stopPropagation()}>{selectedTable ? <><div className="lightbox-head"><div><span className="section-kicker">{selectedTable === "resa" ? "SORTIE" : "ENTRÉE"}</span><h2>{selectedTable === "decades" ? "DECADES" : selectedTable === "synoptic" ? "RENSEIGNEMENTS AGRO" : selectedTable === "normales" ? "NORMALES" : "RESA-01"}</h2></div><button className="icon-button" onClick={() => setSelectedTable(null)} aria-label="Retour">←</button></div><p className="settings-intro">{selectedTable === "normales" ? "Référentiels intégrés par défaut : normales pluviométriques et normales des stations synoptiques." : selectedTable === "resa" ? "Tableau de sortie calculé : NbJP ≥ 1 mm, NbJP > 20 mm, cumul décadaire, écarts, cumul annuel, cumul saisonnier et bilan hydrique." : "Téléversez le fichier correspondant à la décade. Les données sont conservées dans l’historique et utilisées pour recalculer RESA-01."}</p>{(selectedTable === "decades" || selectedTable === "synoptic") && <label className="upload-box">{rainfallUpload === "uploading" ? "Import en cours…" : "Téléverser le fichier Excel de la décade"}<input type="file" accept=".xls,.xlsx" disabled={rainfallUpload === "uploading"} onChange={async (event) => { const file = event.target.files?.[0]; if (!file) return; setRainfallUpload("uploading"); const form = new FormData(); form.set("source", selectedTable === "decades" ? "decades" : "agro"); form.set("file", file); try { const response = await fetch("/api/rainfall", { method: "POST", body: form }); if (!response.ok) throw new Error(); setRainfallUpload("done"); setToast("Décade importée et conservée"); } catch { setRainfallUpload("error"); setToast("Échec de l’import de la décade"); } }} /></label>}<div className="table-preview"><div className="table-preview-head"><span>Poste</span><span>Décade</span><span>NbJP ≥1</span><span>NbJP &gt;20</span><span>Cumul</span><span>Écart</span></div><div className="table-preview-empty">{selectedTable === "normales" ? "Normales intégrées comme valeurs par défaut." : selectedTable === "resa" ? "Aucun calcul disponible avant le premier import des deux entrées." : "Aucune décade téléversée."}</div></div></> : <><div className="lightbox-head"><div><span className="section-kicker">PARAMÈTRES</span><h2>Chaîne pluviométrique</h2></div><button className="icon-button" onClick={() => setShowSettings(false)} aria-label="Fermer"><Icon name="close" /></button></div><p className="settings-intro">Deux fichiers d’entrée par décade alimentent un historique. Les normales restent intégrées par défaut. RESA-01 est une sortie calculée, jamais un fichier à modifier.</p><div className="settings-table-list"><button className="settings-table-item" onClick={() => setSelectedTable("decades")}><strong>1. DECADES</strong><span>Entrée réseau pluviométrique</span></button><button className="settings-table-item" onClick={() => setSelectedTable("synoptic")}><strong>2. RENSEIGNEMENTS AGRO</strong><span>Entrée stations synoptiques</span></button><button className="settings-table-item" onClick={() => setSelectedTable("normales")}><strong>Référentiels NORMALES</strong><span>Données intégrées par défaut</span></button><button className="settings-table-item" onClick={() => setSelectedTable("resa")}><strong>3. RESA-01</strong><span>Sortie automatisée</span></button><button className="settings-table-item" onClick={() => { setShowSettings(false); setShowArchive(true); }}><strong>ARCHIVE DES CARTES</strong><span>Consulter toutes les cartes générées</span></button></div></>}</div></div>}
