...
function Dashboard({ user }: { user: User }) {
  ...
  const [selectedTable, setSelectedTable] = useState<"decades" | "synoptic" | "final" | null>(null);
  ...
  return <main className="app-shell">
    ...
    <section className="workspace-grid">
     ...
     <footer className="footer"><span>Source : USGS FEWS NET · eVIIRS 375 m</span><span>Plateforme opérationnelle <span className="live-dot" /></span></footer>
      {showSettings && <div className="lightbox" role="dialog" aria-modal="true" aria-label="Paramètres et tableaux" onClick={() => setShowSettings(false)}><div className="lightbox-panel tables-panel" onClick={(event) => event.stopPropagation()}>{selectedTable ? <><div className="lightbox-head"><div><span className="section-kicker">TABLEAU</span><h2>{selectedTable === "decades" ? "Cumuls des décades" : selectedTable === "synoptic" ? "Stations synoptiques" : "Tableau final"}</h2></div><button className="icon-button" onClick={() => setSelectedTable(null)} aria-label="Retour">←</button></div><p className="settings-intro">Vue du tableau sélectionné. Les données seront organisées par année, mois et décade.</p><div className="table-preview"><div className="table-preview-head"><span>Année</span><span>Mois</span><span>Décade</span><span>Station / localité</span><span>Cumul</span><span>Max</span></div><div className="table-preview-empty">Aucune donnée chargée pour cette période.</div></div></> : <><div className="lightbox-head"><div><span className="section-kicker">PARAMÈTRES</span><h2>Tableaux pluviométriques</h2></div><button className="icon-button" onClick={() => setShowSettings(false)} aria-label="Fermer"><Icon name="close" /></button></div><p className="settings-intro">Choisissez le tableau à consulter. Les données seront organisées par année, mois et décade; seuls les tableaux autorisés seront éditables.</p><div className="settings-table-list"><button className="settings-table-item" onClick={() => setSelectedTable("decades")}><strong>Cumuls des décades</strong><span>Pluies journalières, cumul décadaire, maximum et jours de pluie</span></button><button className="settings-table-item" onClick={() => setSelectedTable("synoptic")}><strong>Stations synoptiques</strong><span>Données des stations, EW et ETP</span></button><button className="settings-table-item" onClick={() => setSelectedTable("final")}><strong>Tableau final</strong><span>Synthèse des indicateurs pluviométriques et cumuls saisonniers</span></button></div><div className="field-hint">Les fichiers d’août fournis servent de modèles de structure, pas de données fixes.</div></>}</div></div>}
