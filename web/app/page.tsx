...
function Dashboard({ user }: { user: User }) {
  ...
  useEffect(() => { if (!showAgro) return; fetch("/agro/stations").then((r) => r.json()).then((data) => { setAgroStations(data.stations ?? []); setAgroStation(data.stations?.find((s: { principal?: boolean }) => s.principal)?.id ?? data.stations?.[0]?.id ?? ""); }).catch(() => setAgroMessage("Impossible de charger les stations")); }, [showAgro]);
  ...
  return <main className="app-shell">
    ...
    {showAgro && <div className="lightbox" role="dialog" aria-modal="true" onClick={() => setShowAgro(false)}><div className="lightbox-panel agro-panel" onClick={(event) => event.stopPropagation()}><div className="lightbox-head"><h2>Bulletin agrométéo</h2><button className="icon-button" onClick={() => setShowAgro(false)} aria-label="Fermer"><Icon name="close" /></button></div><div className="agro-period"><label>Année<input value={agroYear} onChange={(event) => setAgroYear(event.target.value)} /></label><label>Mois<input value={agroMonth} onChange={(event) => setAgroMonth(event.target.value)} /></label><label>Décade<input value={agroDecade} onChange={(event) => setAgroDecade(event.target.value)} /></label></div><nav className="agro-nav"><button className={agroView === "rain" ? "selected" : ""} onClick={() => setAgroView("rain")}>Saisie pluviométrie</button><button className={agroView === "observations" ? "selected" : ""} onClick={() => setAgroView("observations")}>Renseignements agro</button><button className={agroView === "ewetp" ? "selected" : ""} onClick={() => setAgroView("ewetp")}>ew / ETP</button><button className={agroView === "exports" ? "selected" : ""} onClick={() => setAgroView("exports")}>Exports</button></nav>{agroView === "rain" && <><div className="agro-table-wrap"><table className="agro-table"><thead><tr><th>Station</th>{days.map((day) => <th key={day}>J{day}</th>)}<th>NbJP&gt;0</th><th>NbJP&gt;20</th><th>Max</th><th>Total</th></tr></thead><tbody>{agroStations.map((station) => { const values = days.map((day) => rainRows.find((row) => row.station_id === station.id && row.jour === day)?.hauteur_mm); const valid = values.filter((value): value is number => value !== undefined); return <tr key={station.id}><td>{station.name}</td>{days.map((day) => <td key={day}><input type="number" value={rainRows.find((row) => row.station_id === station.id && row.jour === day)?.hauteur_mm ?? ""} onChange={(event) => updateRain(station.id, day, event.target.value)} /></td>)}<td>{valid.filter((value) => value > 0).length}</td><td>{valid.filter((value) => value > 20).length}</td><td>{valid.length ? Math.max(...valid) : ""}</td><td>{valid.length ? valid.reduce((sum, value) => sum + value, 0) : ""}</td></tr>; })}</tbody></table></div><button className="primary-small" onClick={() => void saveRain()}>Enregistrer les pluies</button></>}{agroView === "observations" && <><label>Station<select value={agroStation} onChange={(event) => setAgroStation(event.target.value)}>{agroStations.filter((station) => station.principal).map((station) => <option key={station.id} value={station.id}>{station.name}</option>)}</select></label><div className="agro-table-wrap"><table className="agro-table"><thead><tr><th>Jour</th>{["pluie", "temp_min", "temp_max", "temp_10cm", "temp_50cm", "vent_moyen", "vent_max", "insolation", "humidite_min", "humidite_max", "tension_vapeur", "evapo_bac_a"].map((key) => <th key={key}>{key}</th>)}<th>T moy</th><th>Hum moy</th></tr></thead><tbody>{days.map((day) => { const row = agroRows.find((item) => item.jour === day) ?? { jour: day }; return <tr key={day}><td>{day}</td>{["pluie", "temp_min", "temp_max", "temp_10cm", "temp_50cm", "vent_moyen", "vent_max", "insolation", "humidite_min", "humidite_max", "tension_vapeur", "evapo_bac_a"].map((key) => <td key={key}><input type="number" value={row[key] ?? ""} onChange={(event) => updateAgro(day, key, event.target.value)} /></td>)}<td>{row.temp_min !== undefined && row.temp_max !== undefined ? (Number(row.temp_min) + Number(row.temp_max)) / 2 : ""}</td><td>{row.humidite_min !== undefined && row.humidite_max !== undefined ? 0.6 * Number(row.humidite_min) + 0.4 * Number(row.humidite_max) : ""}</td></tr>; })}</tbody></table></div><button className="primary-small" onClick={() => void saveObservations()}>Enregistrer les observations</button></>}{agroView === "ewetp" && <div className="table-preview"><p>Tableau ew/ETP des stations principales</p><button className="primary-small">Charger les valeurs ew / ETP</button></div>}{agroView === "exports" && <div className="agro-export-actions"><a href={`/agro/export/network.xlsx?year=${agroYear}&month=${agroMonth}&decade=${agroDecade}`} className="primary-small">Exporter Réseau pluviométrique</a><a href={`/agro/export/climate.xlsx?year=${agroYear}&month=${agroMonth}&decade=${agroDecade}`} className="primary-small">Exporter Données climatiques</a></div>}{agroMessage && <p className="settings-intro">{agroMessage}</p>}</div></div>}
