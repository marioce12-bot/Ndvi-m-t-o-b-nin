...
function Dashboard({ user }: { user: User }) {
  ...
  return <main className="app-shell">
    ...
    {showAgro && <div className="lightbox" role="dialog" aria-modal="true" onClick={() => setShowAgro(false)}><div className="lightbox-panel agro-panel" onClick={(event) => event.stopPropagation()}><div className="lightbox-head"><h2>Paramètres agrométéo</h2><button className="icon-button" onClick={() => setShowAgro(false)} aria-label="Fermer"><Icon name="close" /></button></div><p className="settings-intro">Générez séparément les exports du bulletin pour la décade choisie.</p><div className="agro-period"><label>Année<input value={agroYear} onChange={(event) => setAgroYear(event.target.value)} /></label><label>Mois<input value={agroMonth} onChange={(event) => setAgroMonth(event.target.value)} /></label><label>Décade<input value={agroDecade} onChange={(event) => setAgroDecade(event.target.value)} /></label></div><div className="agro-export-actions"><a href={`${process.env.NEXT_PUBLIC_WORKER_EXPORT_URL ?? ""}/agro/export/network.xlsx?year=${agroYear}&month=${agroMonth}&decade=${agroDecade}`} className="primary-small">Exporter Réseau pluviométrique</a><a href={`${process.env.NEXT_PUBLIC_WORKER_EXPORT_URL ?? ""}/agro/export/climate.xlsx?year=${agroYear}&month=${agroMonth}&decade=${agroDecade}`} className="primary-small">Exporter Données climatiques</a></div></div></div>}
