...
function Dashboard({ user }: { user: User }) {
  ...
  const [newStation, setNewStation] = useState({ name: "", department: "", locality: "" });
  ...
  const createStation = async () => {
    ...
    try {
      await agroFetch("/stations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: stationId(newStation.name), ...newStation }) });
      setAgroStations((stations) => [...stations, { id: stationId(newStation.name), name: newStation.name, department: newStation.department, principal: false }]);
      setAgroMessage("Station ajoutée");
      setNewStation({ name: "", department: "", locality: "" });
  ...
  return <main className="app-shell">
    ...
    {showAgro && <AgroPanel agroYear={agroYear} setAgroYear={setAgroYear} agroMonth={agroMonth} setAgroMonth={setAgroMonth} agroDecade={agroDecade} setAgroDecade={setAgroDecade} agroView={agroView} setAgroView={setAgroView} rainDepartmentGroup={rainDepartmentGroup} setRainDepartmentGroup={setRainDepartmentGroup} visibleRainStations={visibleRainStations} days={days} rainRows={rainRows} updateRain={updateRain} saveRain={saveRain} agroStations={agroStations} agroStation={agroStation} setAgroStation={setAgroStation} agroRows={agroRows} updateAgro={updateAgro} saveObservations={saveObservations} ewEtpRows={ewEtpRows} setEwEtpRows={setEwEtpRows} ewEtpCalculations={ewEtpCalculations} agroMessage={agroMessage} exportAvailability={exportAvailability} agroFetch={agroFetch} setAgroMessage={setAgroMessage} newStation={newStation} setNewStation={setNewStation} createStation={createStation} onClose={() => setShowAgro(false)} />}
