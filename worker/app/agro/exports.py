...
def _append_observations_table(sheet: openpyxl.worksheet.worksheet.Worksheet, year: int, month: int, decade: int, station: Station, rows: list[dict[str, object]]) -> None:
    """Append one full "Renseignements agro" table (title + 15 columns + total/moyenne) for a single station."""
    if sheet.max_row > 1:
        sheet.append([])
    start = sheet.max_row + 1
...
def build_observations_export(year: int, month: int, decade: int, stations: Iterable[Station], rows_by_station: dict[str, list[dict[str, object]]]) -> tuple[BytesIO, str]:
    ...
    list of daily observation rows for the requested decade.
    """
    workbook = openpyxl.Workbook()
    sheet = workbook.active
    ...
    stations = list(stations)
    for station in stations:
        _append_observations_table(sheet, year, month, decade, station, rows_by_station.get(station.id, []))
    sheet.append([])
